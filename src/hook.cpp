...
#undef XX
    ...
    int do_io(int sockfd,Func func,CWJ_CO_NET::IOManager::EventType event_type
...
int usleep(useconds_t usec) {
    ...
    iomanager->addTimer(usec/1000,[co,iomanager](){INFO_LOG(GET_ROOT_LOGGER())<<"eee sleep";iomanager->schedule(co,-1);});
    CWJ_CO_NET::Coroutine::YieldToHold();
    return 0;
}
...
int accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen){
    ERROR_LOG(g_logger)<<"========== test accept ===========";
    return do_io(sockfd, accept_f, CWJ_CO_NET::IOManager::READ,CWJ_CO_NET::FdCtx::RCVTIMEO, addr, addrlen);
}


ssize_t read(int fd, void *buf, size_t count) {
    return do_io(fd, read_f, CWJ_CO_NET::IOManager::READ,CWJ_CO_NET::FdCtx::RCVTIMEO, buf, count);
}

ssize_t readv(int fd, const struct iovec *iov, int iovcnt) {
    return do_io(fd, readv_f,  CWJ_CO_NET::IOManager::READ, CWJ_CO_NET::FdCtx::RCVTIMEO, iov, iovcnt);
}

ssize_t recv(int sockfd, void *buf, size_t len, int flags) {
    return do_io(sockfd, recv_f, CWJ_CO_NET::IOManager::READ, CWJ_CO_NET::FdCtx::RCVTIMEO, buf, len, flags);
}

ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags, struct sockaddr *src_addr, socklen_t *addrlen) {
    return do_io(sockfd, recvfrom_f, CWJ_CO_NET::IOManager::READ, CWJ_CO_NET::FdCtx::RCVTIMEO, buf, len, flags, src_addr, addrlen);
}

ssize_t recvmsg(int sockfd, struct msghdr *msg, int flags) {
    return do_io(sockfd, recvmsg_f, CWJ_CO_NET::IOManager::READ, CWJ_CO_NET::FdCtx::RCVTIMEO, msg, flags);
}

ssize_t write(int fd, const void *buf, size_t count) {
    return do_io(fd, write_f, CWJ_CO_NET::IOManager::WRITE, CWJ_CO_NET::FdCtx::SNDTIMEO, buf, count);
}

ssize_t writev(int fd, const struct iovec *iov, int iovcnt) {
    return do_io(fd, writev_f,  CWJ_CO_NET::IOManager::WRITE, CWJ_CO_NET::FdCtx::SNDTIMEO, iov, iovcnt);
}

ssize_t send(int s, const void *msg, size_t len, int flags) {
    return do_io(s, send_f,  CWJ_CO_NET::IOManager::WRITE, CWJ_CO_NET::FdCtx::SNDTIMEO, msg, len, flags);
}

ssize_t sendto(int s, const void *msg, size_t len, int flags, const struct sockaddr *to, socklen_t tolen) {
    return do_io(s, sendto_f,  CWJ_CO_NET::IOManager::WRITE, CWJ_CO_NET::FdCtx::SNDTIMEO, msg, len, flags, to, tolen);
}

ssize_t sendmsg(int s, const struct msghdr *msg, int flags) {
    return do_io(s, sendmsg_f, CWJ_CO_NET::IOManager::WRITE, CWJ_CO_NET::FdCtx::SNDTIMEO, msg, flags);
}
